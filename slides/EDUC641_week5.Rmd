---
title: "Introduction to Statistical Inference II"
subtitle: "EDUC 641: Week 5"
...
- In a one-sample *z*-test, we assume that the population standard deviation ($\sigma$) is known.
...
```{r, echo=FALSE,eval=TRUE, comment='',fig.width=8,fig.height=6}
...
ggplot() +
  geom_line(aes(x = x, y = y1, color = 'normal'),lwd=1.1) +
  geom_line(aes(x = x, y = y2, color = 't(2)'),lwd=1.1) +
  geom_line(aes(x = x, y = y3, color = 't(10)'),lwd=1.1) +
  geom_line(aes(x = x, y = y4, color = 't(30)'),lwd=1.1) +
  theme_bw() +
  xlab('') +
  ylab('Density') +
...
| -------- | ------------------------------------------------- | ------------------- |
| `dt()`   | Density at a given *t*-score                      | `dt(-1, df = 10)`   |
| `pt()`   | Cumulative probability up to a *t*-score          | `pt(-1, df = 10)`   |
| `qt()`   | Quantile (critical value) for a given probability | `qt(.025, df = 10)` |
...
ggplot()+
  ...
  geom_segment(aes(x=-1,y=0,xend=-1,yend=0.242),lty=2)+
  theme_bw()+
  xlab('z-score')+
  ylab('Density')+
  ggtitle('Standard Normal Dist.')
...
ggplot()+
  geom_line(aes(x=x,y=y)) + 
  geom_segment(aes(x=-1,y=0,xend=-1,yend=0.23),lty=2)+
  theme_bw()+
  xlab('t-score')+
  ylab('Density')+
...
ggplot(NULL, aes(c(-3.5,3.5))) +
  geom_area(stat = "function", fun = dnorm, fill = "#00998a", xlim = c(-3.5,-1)) +
...
ggplot(NULL, aes(c(-3.5,3.5))) +
  geom_area(stat = "function", fun = dt, fill = "#00998a", xlim = c(-3.5,-1),args = list(df = 10)) +
  geom_area(stat = "function", fun = dt, fill = "grey80", xlim = c(-1, 3.5),args = list(df = 10)) +
...
5.Compare with significance level $\alpha$ (e.g., 0.05) and make a decision whether to reject the null hypothesis.
...
**Research question:**
Does a specially designed intervention for mothers of low-birthweight (LBW) infants improve their infants’ later psychomotor development?

**Background:**
Researchers created an intervention program for mothers of LBW infants. The goal was to help mothers become more aware of their infants’ cues and more responsive to their needs—behaviors believed to reduce developmental difficulties later in life.

**Dependent variable:**
Psychomotor development, measured by the Psychomotor Development Index (PDI).
In the general population (normal-weight infants), PDI scores are scaled to have a mean of 100.

**Independent variable:**
Intervention status — whether the mother participated in the program.

**Sample data:**
A researcher collected PDI scores from 56 LBW infants at 6 months old, after their mothers had participated in the intervention shortly after birth.
The sample had the following summary statistics:
...
ggplot(NULL, aes(c(-3.5,3.5))) +
  geom_area(stat = "function", fun = dt, fill = "grey80", xlim = c(-3.5,2.453),args = list(df = 55)) +
  geom_area(stat = "function", fun = dt, fill = "#00998a", xlim = c(2.453, 3.5),args = list(df = 55)) +
  labs(x = "t-score", y = "Density")+
  ggtitle('t-distribution, df = 55')+
  theme_bw()+
  scale_x_continuous(breaks=c(-3,-2,-1,0,1,2,3))
...
$$Cohen'sD =  \frac{\bar{X} - \mu}{\hat{\sigma}} = \frac{104.125 - 100}{12.584} = 0.328 $$ 
...
#### **Example Scenario ([Aronson et al., 1999]**(https://www.sciencedirect.com/science/article/pii/S0022103198913713))
...
- If another researcher replicated the study with similar sample sizes (N₁ = 11, N₂ = 12), 
...
```{r, echo=TRUE,eval=TRUE, comment='', fig.width=5,fig.height=5}
...
```{r, echo=FALSE,eval=TRUE, comment='', fig.width=6,fig.height=6}
# Sampling distribution of differences in sample means
...
knitr::include_graphics(here('resources/images/two-sample.png'))
...
.double[
Given two populations — Population 1 with mean $\mu_1$ and standard deviation $\sigma$,  
and Population 2 with mean $\mu_2$ and standard deviation $\sigma$ —  
the **sampling distribution of the difference in sample means** is approximately normal, with:
...
\hat{\sigma} = 
\sqrt{
  \frac{(n_1 - 1)\hat{\sigma}_1^2 + (n_2 - 1)\hat{\sigma}_2^2}
       {n_1 + n_2 - 2}
}
$$
...
- Because we replace the true population standard deviation with an **estimate**,  
  the sampling distribution of the difference in sample means follows a *t*-distribution
...
$$
   ...
   \sqrt{
     \frac{\hat{\sigma}_p^2}{n_1} + 
     \frac{\hat{\sigma}_p^2}{n_2}
   }
$$
...
**Null Hypothesis ($H_0$)**  
White male college students in the stereotype threat condition perform equally wel on a math exam as those in the control condition.  
...
\hat{\sigma}_p =
\sqrt{
\frac{(11 - 1)(3.17)^2 + (12 - 1)(3.03)^2}{11 + 12 - 2}
}
= 3.097
$$
...
.pull-left[
```{r, echo=FALSE, eval=TRUE, comment='', fig.width=3, fig.height=3}
library(ggplot2)

ggplot(NULL, aes(c(-3.5, 3.5))) +
  geom_area(stat = "function", fun = dt, fill = "grey80",
            xlim = c(-3.5, 2.361), args = list(df = 21)) +
  geom_area(stat = "function", fun = dt, fill = "#00998a",
            xlim = c(2.361, 3.5), args = list(df = 21)) +
  labs(x = "t-score", y = "Density") +
  ggtitle("t-distribution (df = 21)") +
  theme_bw() +
  scale_x_continuous(breaks = c(-3, -2, -1, 0, 1, 2, 3))
...
(the white male college students in the stereotype threat condition perform equally well as those in the control condition),
the probability of observing a mean difference of 3.06 points or more extreme is 0.014.

<br>

At a significance level of 0.05, we reject the null hypothesis.
This suggests that the observed data are not compatible with the assumption that both groups perform equally well.
...
$$
Cohen's\ d = 
\frac{|\bar{X}_1 - \bar{X}_2|}{\hat{\sigma}_p} =
\frac{|9.64 - 6.58|}{3.097} = 0.988
$$

A **Cohen’s d = 0.99** indicates a **large effect size**,  
suggesting that the difference in performance between the two groups is substantial.
...
- However, it is possible to perform an independent samples *t*-test **without** assuming equal variances:  
  ...
  - The formulas for the test statistic and degrees of freedom differ slightly  
    (see [Ch. 13.4 in *Learning Statistics with R*](https://learningstatisticswithr-bookdown.netlify.app/ttest.html#welchttest)).  
...
An independent-samples **Welch’s *t*-test** was conducted to examine whether students in the experimental condition scored significantly lower than those in the control condition.  
Results indicated that the mean test score in the experimental condition was 3.06 points lower than in the control condition, a difference that was statistically significant, *t*(20.61) = 2.356, *p* = .014, Cohen’s *d* = 0.99.
...
knitr::include_graphics(here('resources/images/dependent.png'))
...
$$
Cohen's\ d = \frac{\bar{X}_D}{\hat{\sigma}_D}
= \frac{7.26}{7.16}
= 1.01
$$
